...
this.Tips = new Class({
	...
	elementEnter: function(event, element){
		...
		this.timer = (function(){
			...
			['title', 'text'].each(function(value){
				...
				events.each(function(value){
					var event = div.retrieve('tip-content:' + value);
					if (!event) event = function(evt){
						this['tip' + value.capitalize()].apply(this, [event, element]);
					}.bind(this);

					div.store('tip:' + value, event).addEvent('mouse' + value, event);
				}, this);
			}, this);
	...
	tipEnter: function(event, element) {
		if (!this.onTipContent) {
			this.onTipContent = true;
			this.elementEnter(event, element);
		}
	}, 
	
	tipLeave: function(event, element) { 
		this.onTipContent = false;
